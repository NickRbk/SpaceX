...
public interface IPotentiallyFlyable {
    default boolean isPotentiallyFlyable(BigDecimal distanceMeters, float acceleration, int totalWeight, float efficiency, int totalFuel) {
        ...
        if(BigDecimal.valueOf(totalFuel).compareTo(fuelNeeds) > 0) {
            ...
            System.out.println("You haven`t enough fuel for travel, you need more " + fuelNeeds.subtract(BigDecimal.valueOf(totalFuel)).toBigInteger());
            return false;
        }
